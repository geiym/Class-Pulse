...
public class AnalyticsEngine {
        ...
        for (String d : days) {
            for (String s : slots) {
                ...
                if (scores == null || scores.isEmpty()) {
                    ...
                    // Kapag ang average ay >= 2.5, "High" ang level.
                    String level;
    ...
    // ─────────────────────────────────────────────────
    // ─────────────────────────────────────────────────
    // RULE 1: WEIGHTED SCORING (Pag-compute ng Grade/Score)
    // Dito natin tinutukoy kung gaano ka-engaged ang estudyante sa isang subject.
    // Ito ang formula kung paano nako-compute ang "Engagement Score".
    // ─────────────────────────────────────────────────
    public static List<SubjectEngagement> computeSubjectRankings(List<ClassLog> logs, List<Subject> subjects) {
        // RULE: Bucketing (Paglalagay sa tamang kategorya)
        ...
        for (Subject subject : subjects) {
            ...
            for (ClassLog log : subLogs) {
                // RULE: Weighted Calculation
                // Attendance (0-2) * 20 = Max 40 points
                // Participation (0-3) * 20 = Max 60 points
                // Notes (May sulat o wala) = Max 10 points
                // Ang total (110) ay ini-scale para maging percentage (Max 100%).
        ...
        // RULE: Sorting (Pag-rank mula pinakamataas hanggang pinakamababa)
        Collections.sort(result, (a, b) -> Double.compare(b.engagementScore, a.engagementScore));
        return result;
    }

    // ─────────────────────────────────────────────────
    //  4. MID-WEEK SLUMP DETECTION ( Ang rule na ito ay naghahanap ng pattern kung kailan mababa ang performance.)
    // ─────────────────────────────────────────────────
    // 1. Group all logs by Day of Week (Mon-Fri)
    // 2. Calculate the average participation score for each day
    // 3. RULE: If the lowest day score is < 1.5, identify it as a Slump.
    public static String detectMidWeekSlump(List<ClassLog> logs) {
        ...
        // 3. CALCULATION OF AVERAGES (Pagkuha ng Average)
        // Dito natin kinukuha ang "Mean" o average ng participation score sa bawat araw.
        ...
        // 4. FINDING THE MINIMUM (Paghahanap ng pinakamababa)
        // RULE: Hinahanap kung aling araw ang may absolute lowest average score
        ...
        // 5. THE HEURISTIC RULE (Ang Rule-Based Decision)
        // Dito nagaganap ang "Logic":
        // Hindi sapat na ito ang pinakamababa; dapat ay mas mababa ito sa THRESHOLD na 1.5.
        // Kapag ang score ay less than 1.5, ibig sabihin ay "Below Average" ang performance sa araw na iyon.
    ...
    // ─────────────────────────────────────────────────
// RULE 4: COMPARISON RULE (Trajectory Analysis)
// Kinukumpara ang huling 14 days vs previous 14 days.
// ─────────────────────────────────────────────────
    public static TrajectoryResult computeTrajectory(List<ClassLog> allLogs) {
        ...
        // ... filtering recent and previous logs ...

        // RULE: Delta Calculation
        // Kinukuha ang difference (delta) ng dalawang periods.
        ...
        // Kapag ang improvement ay > 0.25, ang status ay "Improving".
        if (delta > 0.25) {
...
// ─────────────────────────────────────────────────
// RULE 5: DECISION MATRIX RULE (Smart Feedback)
// Ito ang "Brain" ng app na nagpapasya kung anong tips ang lalabas.
// ─────────────────────────────────────────────────
    public static SmartFeedback generateSmartFeedback(List<ClassLog> logs, List<Subject> subjects) {
        // RULE: Guard Condition (Kailangan ng minimum data)
        if (logs.size() < 3) {
            return new SmartFeedback("Keep logging your classes to unlock personalized insights!");
        }

        // RULE: Time-Slot Comparison (Umaga vs Hapon)
        ...
        // RULE: Tired Mood Pattern (Repetition Rule)
        // Kapag na-detect na 3 beses ka nang "Tired" at "Low" participation.
        if (tiredLow >= 3) {
            return new SmartFeedback(
    ...
    // ─────────────────────────────────────────────────
// RULE 6: MAPPING RULE (TimeSlot & Days)
// Simpleng conversion ng numerical data (hour/day) tungo sa readable labels.
// ─────────────────────────────────────────────────
